...
# Final standalone analysis version v1.1.10 (analytical results unchanged)
# Second-round reviewer presentation patch: figures/labels only, 2026-08-25
...
presentation_patch <- "R2_FIGURE_PATCH_FINAL_2026-08-25"
analysis_package_name <- paste0(
  "GBD_EOBC_final_code_package_",
  analysis_version,
  "_R2_FIGURE_PATCH_FINAL"
)
...
p_map_inc  <- plot_single_map("Incidence", "A1. Global age-standardized incidence rate", "ASIR\nper 100,000")
p_map_prev <- plot_single_map("Prevalence", "A2. Global age-standardized prevalence rate", "ASPR\nper 100,000")
p_map_dea  <- plot_single_map("Deaths", "A3. Global age-standardized death rate", "ASDR\nper 100,000")
p_map_daly <- plot_single_map("DALYs", "A4. Global age-standardized DALY rate", "AS-DALY\nper 100,000")
...
p1 <- plot_trend("Incidence", "B1. ASIR trends: China, Global, and SDI strata", "Age-standardized rate per 100,000")
p2 <- plot_trend("Deaths", "B2. ASDR trends: China, Global, and SDI strata", "Age-standardized rate per 100,000")
p3 <- plot_trend("DALYs", "B3. AS-DALY trends: China, Global, and SDI strata", "Age-standardized rate per 100,000")
...
china_total_increase_fig2 <- china_res$Cases_2023 - china_res$Cases_1990

df_china_components_fig2 <- tibble(
  Component = factor(
    c(
      "Population\ngrowth",
      "Age-structure\nchange",
  ...
  mutate(
    Share = Contribution / china_total_increase_fig2,
    Label = paste0(
      comma(round(Contribution)),
      "\n(",
...
p_components <- ggplot(
  df_china_components_fig2,
  aes(x = Component, y = Contribution, fill = Component)
) +
  geom_col(
    color = "black",
    linewidth = 0.4,
  ...
  labs(
    title = "A. Components of increased incident cases in China",
    x = "",
    y = "Increase in incident cases, n"
  ) +
  theme_pub(13) +
  theme(legend.position = "none")
...
p_bar <- ggplot(
  ...
  labs(
    title = "B. Relative contributions: China, Global, and SDI strata",
    x = "",
...
df_cohort <- df_full %>%
  ...
  mutate(
    ...
    cohort_group = factor(
      cohort_group,
      levels = c("1960s", "1970s", "1980s", "1990 or later")
    )
  ) %>%
...
p_cohort <- ggplot(
  df_cohort,
  aes(x = age_name, y = mean_incidence_rate, fill = cohort_group)
) +
  geom_col(
  ...
  scale_fill_manual(values = c(
    ...
    "1970s" = "#4DBBD5FF",
    "1980s" = "#E18727FF",
    "1990 or later" = "#BC3C29FF"
  )) +
  scale_x_discrete(drop = FALSE) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.08))) +
  labs(
    title = "C. Descriptive cohort-specific incidence patterns in China",
    x = "Age group",
...
fig2_final <- wrap_elements(
  p_components + p_bar + plot_layout(widths = c(1, 1))
) /
...
p_macro <- ggplot(df_macro, aes(x = year, y = val, color = rei_name, linetype = location_name)) +
  ...
  labs(title = "A. Broad risk-attribution trends: China and Global", x = "", y = "Descriptive mean PAF", color = "Risk category", linetype = "Location") +
  theme_pub(12) +
  theme(legend.position = "bottom")
...
p_micro_global <- plot_heatmap("Global", "C. Specific risk-attribution heatmap: Global", FALSE)
p_micro_china <- plot_heatmap("China", "D. Specific risk-attribution heatmap: China", TRUE)
...
p_shap <- sv_importance(shp, kind = "beeswarm") +
  ...
  labs(title = "B. China-specific SHAP summary for exploratory ranking", x = "SHAP value: contribution to predicted DALYs in thousands") +
...
df_paf_2023 <- df_micro %>%
  ...
  left_join(
    df_micro %>%
      ...
      summarise(`Descriptive mean of age-specific PAFs` = mean(val, na.rm = TRUE), .groups = "drop"),
    by = "Risk_Factor"
  ) %>%
...
p_mir <- ggplot() +
  ...
  scale_y_continuous(labels = percent_format(accuracy = 1)) +
  labs(
    title = "A. MIR trajectories: China, Global, and SDI strata",
    subtitle = "Ratio of recalculated weighted age-standardized death rate to incidence rate",
    x = "", y = "MIR"
  ) +
  theme_pub(13)
...
p_bar_total <- ggplot(df_total_2023, aes(x = age_name, y = Total_Loss_Billion)) +
  ...
  labs(
    title = "B. China: estimated mortality-related productivity loss in 2023",
...
p_risk_trend <- ggplot(df_risk_trend, aes(x = year, y = Loss_Billion, color = Risk_Factor, group = Risk_Factor)) +
  ...
  labs(title = "C. China: risk-attributable mortality-related productivity losses", x = "Year", y = "Attributable loss, billion constant 2015 US$") +
  theme_pub(13) +
  theme(legend.position = "none")
...
p_partA_f5 <- plot_dual_gradient(
  df_final_fig5 %>%
    ...
    mutate(age_name = factor(age_name, levels = age_specific)),
  "A. China and Global age-specific incidence-rate projections to 2050"
) +
  facet_wrap(~ age_name, ncol = 3, scales = "free_y")

p_partB_f5 <- plot_dual_gradient(
  df_final_fig5 %>% filter(age_name == "Overall ASIR (25-49y)"),
  "B. China and Global overall ASIR projections, ages 25–49",
  center_title = TRUE
)
...
p_val_bar <- ggplot(
  ...
  labs(
    title = paste(
      "A. China and Global: GBD rates vs rates derived from",
...
p_val_scatter <- ggplot(
  ...
  labs(
    title = "B. China and Global GBD–GLOBOCAN age-pattern comparison",
...
main_figure_captions_v1_1_10 <- c(
  paste(
    ...
    "The upper panels show global all-age age-standardized burden rates in 2023.",
    "The lower panels show author-recalculated weighted rates among women aged",
    "25-49 years across China, the global population, and five SDI strata;",
    "panel B4 shows the YLL/YLD composition of DALYs in China.",
    "ASIR, age-standardized incidence rate; ASPR, age-standardized prevalence rate;",
    "ASDR, age-standardized death rate; DALY, disability-adjusted life year;",
    "YLL, year of life lost; YLD, year lived with disability;",
    "SDI, Socio-demographic Index."
  ),
  paste(
    "Figure 2. Components of increased incident cases and descriptive",
    "cohort-specific incidence patterns. Panel A displays the three China",
    "decomposition components as independent bars, with case contributions and",
    "shares of the total increase labelled directly; it is not a waterfall plot.",
    "Panel B displays each signed component divided by the same total increase",
    "for China, the global population, and the five SDI strata; negative values",
    "therefore remain negative. Panel C uses grouped bars to show descriptive",
    "cohort-specific incidence patterns; later cohorts are shown only for ages",
    "observable within 1990-2023. SDI, Socio-demographic Index."
  ),
  paste(
    "Figure 3. Risk-attribution patterns and exploratory feature ranking.",
    "Panel A shows descriptive unweighted arithmetic means of broad DALY PAF",
    "categories across the five age groups for China and the global population;",
    "these means are descriptive summaries and are not overall population-weighted",
    "PAFs for women aged 25-49 years. Panel B shows the China-specific SHAP summary",
    "for the four nonconstant DALY PAF features. Panel C shows global signed",
    "risk-specific DALY PAFs by age and year, followed by China in panel D.",
    "High BMI was excluded from ranking because its China series was constant at",
    "zero. PAF, population attributable fraction; SHAP, SHapley Additive",
    "exPlanations; BMI, body mass index; DALY, disability-adjusted life year."
  ),
  paste(
    "Figure 4. Mortality-to-incidence ratios and mortality-related productivity",
    "losses. Panel A shows weighted MIR trajectories for ages 25-49 years across",
    "China, the global population, and five SDI strata. Panels B and C report",
    "China-specific Human Capital Approach estimates in constant 2015 US dollars",
    "using YLL-specific PAFs. MIR, mortality-to-incidence ratio; ASDR,",
    "age-standardized death rate; ASIR, age-standardized incidence rate;",
    "HCA, Human Capital Approach; YLL, year of life lost; PAF, population",
    "attributable fraction; SDI, Socio-demographic Index."
  ),
  paste(
    "Figure 5. Status-quo Bayesian age-period-cohort projections of incidence",
    "to 2050 for China and the global population. The dotted line marks the 2023",
    "transition. Shaded bands represent 80% and 95% posterior uncertainty",
    "intervals after multiplicative anchoring of posterior means and interval",
    "bounds to observed 2023 rates. ASIR, age-standardized incidence rate."
  ),
  paste(
    "Figure 6. External comparison of GBD 2022 incidence rates and rates derived",
    "from GLOBOCAN 2022 age-specific case estimates for China and the global",
    "population. GLOBOCAN case estimates were divided by the matched study",
    "population denominators to construct comparison rates. Pearson correlation",
    "summarizes age-pattern concordance and is not formal external validation of",
    "the BAPC model. GBD, Global Burden of Disease; BAPC, Bayesian age-period-cohort."
  )
)
